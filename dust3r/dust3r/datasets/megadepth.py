...
def _parse_scene(scene_str):
    scene_str = _to_str(scene_str)
    if '/' in scene_str:
        parts = [p for p in scene_str.split('/') if p]
    else:
        parts = [p for p in scene_str.split() if p]
    if not parts:
        raise ValueError(f'bad scene string: {scene_str!r}')
    if len(parts) == 1:
        return parts[0], ''
    return parts[0], parts[1]


def _resolve_existing(seq_path, candidates):
    for name in candidates:
        if not name:
            continue
        path = osp.join(seq_path, name)
        if osp.exists(path):
            return path
    return None
...
class MegaDepth(BaseStereoViewDataset):
    ...
    def _load_data(self, split):
        with np.load(osp.join(self.ROOT, 'all_metadata.npz'), allow_pickle=True) as data:
    ...
    def _get_views(self, pair_idx, resolution, rng):
        ...
        for _ in range(max_retries):
            try:
                return self._get_views_once(pair_idx, resolution, rng)
            except (OSError, IOError, ValueError) as e:
                last_error = e
                pair_idx = int(rng.integers(0, len(self.pairs)))
        raise last_error

    def _get_views_once(self, pair_idx, resolution, rng):
        scene_id, im1_id, im2_id, score = self.pairs[pair_idx]
        scene_id = int(scene_id)
        im1_id = int(im1_id)
        im2_id = int(im2_id)

        scene, subscene = _parse_scene(self.all_scenes[scene_id])
        seq_path = osp.join(self.ROOT, scene, subscene)

        views = []

        for im_id in [im1_id, im2_id]:
            img = _to_str(self.all_images[im_id])
            stem = osp.splitext(img)[0]
            rgb_path = _resolve_existing(seq_path, [
                img,
                img + '.jpg',
                img + '.jpg.jpg',
                stem + '.jpg',
            ...
            exr_path = _resolve_existing(seq_path, [
                img + '.exr',
                img + '.jpg.exr',
                stem + '.exr',
                stem + '.jpg.exr',
            ])
            npz_path = _resolve_existing(seq_path, [
                img + '.npz',
                img + '.jpg.npz',
                stem + '.npz',
                stem + '.jpg.npz',
            ])
            if rgb_path is None or exr_path is None or npz_path is None:
                missing = []
                if rgb_path is None:
                    missing.append('rgb')
                if exr_path is None:
                ...
                if npz_path is None:
                    missing.append('npz')
                raise FileNotFoundError(f'missing {", ".join(missing)} for {img} in {seq_path}')
            try:
                image = imread_cv2(rgb_path)
                depthmap = imread_cv2(exr_path)
